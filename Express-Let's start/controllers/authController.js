...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (freshUser.changePasswordAfter(decoded.iat)) {
    return next(
      new AppError('User recently changed password! Please log in agein.', 401)
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('There is no user with emai address', 404));
  }
  ...
  const text = `Forgot your password? Submit a PATCH request with your new password and passswordConfirm to:
   ${resetURL}.\n
